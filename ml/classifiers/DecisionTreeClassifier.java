...
public class DecisionTreeClassifier implements Classifier{
	...
	public void train(DataSet data) {
		...
		this.allData = data;
		// we'll just use the first label as the parent label to get things going
	...
	private DecisionTreeNode buildTree(ArrayList<Example> currentData, HashSet<Integer> usedFeatures, int depthLimit){
		...
		// 3. we're out of features to examine
		if( majority.majorityCount == currentData.size() ||
			usedFeatures.size() == allData.getAllFeatureIndices().size() ||
			depthLimit == 0){
			return new DecisionTreeNode(majority.majorityLabel);
		}else{
			...
			// Handle base cases 2 and 4 here before making a recursive call
			if( splits[0].size() == 0 ||
			    allExamplesEqual(splits[0] )){
				node.setLeft(new DecisionTreeNode(majority.majorityLabel));
			}else{
			...
			// right branch
			if( splits[1].size() == 0 ||
				allExamplesEqual(splits[1]) ){
				node.setRight(new DecisionTreeNode(majority.majorityLabel));
			}else{
	...
	private int getBestFeatureIndex(ArrayList<Example> currentData, HashSet<Integer> usedFeatures){
		...
		for( int featureIndex: allData.getAllFeatureIndices()){
	...
	/**
	 * @param data
	 * @return Returns true if all of the examples have the exact same feature values
	 */
	private boolean allExamplesEqual(List<Example> data) {
		Example first = data.get(0);
		
		for(Example e: data) {
			if( !examplesFeaturesEqual(first, e) ) {
				return false;
			}
		}
	 ...
	 * @param e1
	 * @param e2
	 * @return true if both examples have the same feature values.  Assumes that they have the same features.
	 */
	private boolean examplesFeaturesEqual(Example e1, Example e2) {
		for(int f: e1.getFeatureSet() ) {
			if( e1.getFeature(f) != e2.getFeature(f) ) {
				return false;
			}
		}
	...
	public String toString(){
		return decisionTree.treeString(allData.getFeatureMap());
	}
	...
	/**
	 * A container class to allow us to return multiple values when calculating
